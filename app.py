...
if selected2 == 'Home':
    ...
    login = st.button('Go To Prediction')
    if login:
        webbrowser.open("https://share.streamlit.io/blessedmut/time-series-crypto/main/app.py#historical-price-data")

    #defining ticker variabel
    Bitcoin = 'BTC-USD'
    Ethereum = 'ETH-USD'
    Ripple = 'XRP-USD'
    BitcoinCash = 'BCH-USD'
    ...
    BTC = yf.download(Bitcoin, start="2022-06-18",end="2022-06-18")
    ETH = yf.download(Ethereum, start="2022-06-18",end="2022-06-18")
    XRP = yf.download(Ripple, start="2022-06-18",end="2022-06-18")
    BCH = yf.download(BitcoinCash, start="2022-06-18",end="2022-06-18")
    ...
    imageBCH = Image.open(urlopen('https://s2.coinmarketcap.com/static/img/coins/64x64/1.png'))
    st.image(imageBCH)
    st.table(BCH)                
...
if selected2 == "Buzzing News⚡":
    ...
    url = "https://economictimes.indiatimes.com/markets/cryptocurrency/rssfeeds/82519373.cms"
    f = feedparser.parse(url)
    
    
    with st.expander('Expand for Crypto Market news'):
